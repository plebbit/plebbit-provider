...
if [ -f .deploy-env ]; then
  export $(echo $(cat .deploy-precache-gateway-env | sed 's/#.*//g'| xargs) | envsubst)
fi
...
for FILE_NAME in ${FILE_NAMES[@]}; do
  sshpass -p "$DEPLOY_PASSWORD" scp $FILE_NAME "$DEPLOY_USER"@"$DEPLOY_HOST":/home/precache-gateway
done

SCRIPT="
cd /home/precache-gateway
scripts/precache-gateway-docker.sh
"
